...
void set_left_motor_speed(unsigned int sp, unsigned char dir)
{
	...
	if(dir == FORWARD)	// Forward motion of the wheel
	{
    ...
    //unsigned int spd = (unsigned int) map_float(sp, 0, 1023, 0, 990);
    if(sp < 990)
	...
	{
    unsigned int spd = (unsigned int) map_float(sp, 0, 1023, 0, 780);
		if(sp < 990)
			duty_cycle = 50 - ((50 * spd) >> 10);
		else
			duty_cycle = 10;		// Minimum allowed Duty Cycle for the motor
	}
...
{
  if(angle == 0) /// 0 deg rotation
  {
    set_servo_DC(HOME);
    return;
  }
  if(angle == 90) /// 90 deg rotation
  {
    set_servo_DC(FRONT); 
    return;
  }
  if(angle == 180) /// 180 deg rotation
  {
    set_servo_DC(MAX); 
    return;
  }
