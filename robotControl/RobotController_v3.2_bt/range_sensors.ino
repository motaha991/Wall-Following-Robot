...
int init_range_sensors(void)
{
  ...
  for (uint8_t i = 0; i < NUM_SENSORS; i++)
  {
    ...
    sensor->init();
    sensor->setTimeout(95);
    sensor->setMeasurementTimingBudget(88000);  // Timing budget in Micro-Seconds. Change the Timeout accordingly as well.
 ...
 */
double get_orientation(uint8_t dir)
{
  double ornt = 0.0;
  if(dir == RIGHT)
  {
    ornt = (((((double)ranges[S1R]) - (double)(ranges[S2R])))/10.0); // S1R - S2R
  }
  else if(dir == LEFT)
  {
    ornt = (((((double)ranges[S2L]) - (double)(ranges[S1L])))/10.0); // S2L - S1L
  }
  else
  {
    ornt = (((((double)ranges[S1R] - (double)ranges[S2R])) + (((double)ranges[S2L] - (double)ranges[S1L])))/20.0); //average of both left and right orientations
  }
  return(ornt);
}
