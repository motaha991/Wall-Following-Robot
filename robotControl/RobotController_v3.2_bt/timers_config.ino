...
{
	DDRB |= (1<<PB1);		// Configure OC1A for output
	DDRB |= (1<<PB2);		// Configure OC1B for output
	...
	OCR1B = 127;
  //TIMSK1 |= (1<<TOIE1); /// Servo Control usge
  DDRC |= (1<<PC0); // set this pin to output for Servo Control
}
...
void set_servo_DC(int fall_edg)
{
  falling_edge = fall_edg;
}
ISR(TIMER1_OVF_vect)
{
  interrupt_count ++;
  if(interrupt_count == falling_edge)
    PORTC &= ~(1<<PC0);
