...
int init_range_sensors(void)
{
  ...
  for (uint8_t i = 0; i < NUM_SENSORS; i++)
  {
    ...
    sensor->setMeasurementTimingBudget(75000);  // Setting the timing budget here (in Micro-Seconds). Greater value = Greater accuracy
...
{
  Serial.println("Starting range sensors calibration:");
  for(int i=0; i<NUM_SENSORS; i++)
    calib_offsets[i] = 0;       // Initialize the offsets with 0s
  /********************** Calibrate Right Side Sensors *********************************/
  Serial.println("Place the robot's Right side touching a wall and press Right Button.");
  turn_LED_on(BLUE_LED);
  delay(200);
  while(!button_is_pressed(RIGHT_BUTTON));  // Wait till the user presses the Right Button
  ...
  {
    int reading_sum = 0;
    for(int x = 0; x<8; x++)  // Take average of 8 readings
    {
    ...
    reading_sum = (reading_sum >> 3); // Take average of the 8 values
    Serial.print("Avg range at sensor ");
    Serial.print(i);
    Serial.print(": ");
    Serial.print(reading_sum);
    if(reading_sum < 120)  // There is no error in the range reading
    {
      Serial.print(" Offset: ");
  ...
  while(!button_is_pressed(RIGHT_BUTTON));  // wait untill the user presses the button
  turn_LED_off(BLUE_LED);
  delay(200);
  ...
  while(!button_is_pressed(RIGHT_BUTTON));  // Wait till the user presses the button
  turn_LED_off(BLUE_LED);
  Serial.println("Calibrating Front sensors ...");
  delay(1000);
  for(int i = 4; i < 5; i++)
  {
    int reading_sum = 0;
...
void print_range_values_only(void)
{
  Serial.println("");
  for(int i=0; i<NUM_SENSORS; i++)
  {
